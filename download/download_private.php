<?php
/**
 * @package bitrix/wizards/bitrix/demo/modules/examples/public/language/ru/examples/download
 */
...
$DIR = DIRECTORY_SEPARATOR . explode('/', $sCurUrl)[1];
$file = substr($sCurUrl, strlen(dirname($sCurUrl)) + 1);
$path = dirname(substr($sCurUrl, strlen(explode('/', $sCurUrl)[1]) + 1)) . DIRECTORY_SEPARATOR;
$filename = $_SERVER["DOCUMENT_ROOT"] . $path . $file;

if (file_exists($filename) && is_file($filename)) {
    require_once($_SERVER["DOCUMENT_ROOT"] . "/bitrix/modules/main/include/prolog_before.php");
    @set_time_limit(0);
    $FILE_PERM = $APPLICATION->GetFileAccessPermission($path . $file, $USER->GetUserGroupArray());
    $FILE_PERM = (strlen($FILE_PERM) > 0 ? $FILE_PERM : "D");
    if ($FILE_PERM < "R")
        LocalRedirect($DIR . "/auth.php?fname=" . urlencode($file) . "&DIR=" . $path);
    else {
